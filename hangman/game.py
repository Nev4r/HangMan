...
from hangman.InvalidOperationError import InvalidOperationError
...
class Game:
    ...
    def __init__(self, allowed_misses: int = 6):
        if allowed_misses < 5 or allowed_misses > 8:
            raise ValueError("Количество попыток должно быть между 5 и 8.")
    ...
    def guess_letter(self, letter: str) -> Iterable[str]:
        if self.__tries_counter == self.allowed_misses:
